...
struct ToyInlinerInterface : public DialectInlinerInterface {
  ...
  /// This hook checks if 'sourceType' and 'targetType' are convertible or if
  /// an argument or result type mismatch prevents inlining. For Toy, all types
  /// are convertible, so we simply return true.
...
With a proper cast operation, we can now override the necessary hooks on the
