...
bool InlinerInterface::isTypeConvertible(Operation *call, Operation *callable,
                                         ...
                                         bool isResult) const {
  if (auto *handler = getInterfaceFor(callable))
...
/// Converts an array attribute to an array of non-null attribute dictionaries.
/// Returns an array of empty dictionaries if the array attribute is null.
static SmallVector<DictionaryAttr>
getAttributesImpl(CallableOpInterface callable, size_t arraySize,
                  ArrayAttr arrayAttr) {
  if (!arrayAttr)
    return SmallVector<DictionaryAttr>{
        arraySize, DictionaryAttr::get(callable.getContext(), {})};
  assert(arraySize == arrayAttr.size());
  return llvm::to_vector(arrayAttr.getAsRange<DictionaryAttr>());
}

/// Returns the array of argument attribute dictionaries.
static SmallVector<DictionaryAttr>
getArgumentAttributes(CallableOpInterface callable) {
  return getAttributesImpl(callable,
                           callable.getCallableRegion()->getNumArguments(),
                           callable.getCallableArgAttrs());
}

/// Returns the array of result attribute dictionaries.
static SmallVector<DictionaryAttr>
getResultAttributes(CallableOpInterface callable) {
  return getAttributesImpl(callable, callable.getCallableResults().size(),
                           callable.getCallableResAttrs());
}
...
LogicalResult mlir::inlineCall(InlinerInterface &interface,
  ...
  return inlineRegionImpl(interface, src, call->getBlock(),
